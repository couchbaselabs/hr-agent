...
if [ -n "$CBCERT" ]; then
    echo "$CBCERT" > /app/couchbase-root-cert.pem
fi
...
exec gunicorn -k uvicorn.workers.UvicornWorker -w "${WORKERS}" -b "${BIND}" --timeout "${TIMEOUT}" --keep-alive "${KEEPALIVE}" svc.main:app
